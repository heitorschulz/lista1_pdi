...
def exercicio4(imagem1, imagem2):
    ...
    img_c1 = centralizar(imagem1)
    img2_c1 = centralizar(imagem2)
    ...
    plt.subplot(231), plt.imshow(imagem1, "gray"), plt.title("Original Image")
    ...
    for index in range(512*512):
        ...
        img1_forma_polar = cmath.polar(img_c2[int(index/512),index%512])
        img2_forma_polar = cmath.polar(img2_c2[int(index/512),index%512])
        ...
        img_c2[int(index/512),index%512] = cmath.rect(novo_valor[0],novo_valor[1])


    plt.subplot(235), plt.imshow(np.log(np.abs(img_c2)), "gray"), plt.title("Spectrum Modified")
    plt.subplot(236), plt.imshow(np.angle(img_c2), "gray"), plt.title("Phase Angle Modified")

    img_c3 = np.fft.ifft2(img_c2)

    img_c4 = descentralizar(img_c3)

    plt.subplot(234), plt.imshow(np.abs(img_c4), "gray"), plt.title("Processed Image")
